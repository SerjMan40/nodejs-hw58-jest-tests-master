...
async function compressFile(filePath) {
  ...
  try {
    await pipe(createReadStream(filePath), createGzip(), createWriteStream(compressedFilePath))
...
async function decompressFile(compressedFilePath, destinationFilePath) {
  ...
  try {
    await pipe(createReadStream(compressedFilePath), createGunzip(), createWriteStream(uniqueDestinationPath))
...
async function performCompressionAndDecompression() {
  try {
    ...
    const decompressedResult = await decompressFile(compressedResult, './src/files/source_decompressed.txt')
